...
String requestAuth(char *skserver, int skport, char *skpath)
{
  ...
  String body = "{ \"clientId\": \"" + String(device_name) + "\", \"description\": \"Engine\",\"permissions\": \"readwrite\"}";
...
void sendData(double f)
{
  ...
  String engineState = fabs(f) < 0.1 ? "stopped" : "started";

  String s = update1 + me + update2 + String(f) + update21 + engineState + update4;
  client.send(s);
  if (DEBUG)
  {
    Serial.print("Sent: ");
